...
| ~16:05 | Fast-forward push → `434a021`: **Step 10 start** — full README (architecture diagram, MotionCommand contract, DLS IK, PIN, voice grammar+agent, run) + **security fix**: `.env.example` had a real-looking old `gsk_` key → replaced with placeholder. Live key never committed (`.env.local` git-ignored, verified). User revoked the old key at Groq. Old key still in history (`43db398`/`100f638`) but now dead — no history rewrite (teammates pushing, too close to freeze). | Arif (push request) |
| ~15:41 | Fast-forward push → `3b06af2`: event-log layout fix — panel was collapsing to ~26px (feed 0px) from `flex-1/min-h-0` inside the scrolling sidebar; events were logging fine all along. Now `max-h-64` bounded feed + live event-count badge. Build green, 50/50 tests. | Arif (push request) |
